...
function onMove(index: number, direction: 'forward' | 'backward') {
  if (direction === 'forward' && index !== 0 && index < images.value.length) {
    const currentImage = images.value[index]
    if (currentImage !== undefined) {
      const moved = images.value.splice(index - 1, 1, currentImage)[0]
      if (moved !== undefined) {
        images.value[index] = moved
      }
    }
  }
  if (direction === 'backward' && index !== images.value.length - 1 && index < images.value.length) {
    const currentImage = images.value[index]
    if (currentImage !== undefined) {
      const moved = images.value.splice(index + 1, 1, currentImage)[0]
      if (moved !== undefined) {
...
<template>
  <div class="space-y-2">
    ...
    <div v-if="!props.hideTips" class="flex flex-wrap gap-1 text-xs text-card-foreground/50 empty:hidden">
