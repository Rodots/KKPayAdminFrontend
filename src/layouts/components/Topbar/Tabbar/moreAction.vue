...
const resultList = computed(() => {
  ...
  result = tabbarStore.list.filter((item) => {
    let flag = false
    if (
      (item.title?.toString() ?? '').includes(searchInput.value)
      || (match((item.title?.toString() ?? '').toString(), searchInput.value, { continuous: true })?.length ?? 0) > 0
    ) {
      flag = true
    }
    return flag
...
<template>
  <FaDropdown
    v-model:open="open" align="end" :items="[
      [
        { label: '关闭其它标签页', icon: 'i-mdi:close', handle: () => tabbar.closeOtherSide(), disabled: !tabbar.checkCloseOtherSide() },
        { label: '关闭左侧标签页', icon: 'i-mdi:arrow-expand-left', handle: () => tabbar.closeLeftSide(), disabled: !tabbar.checkCloseLeftSide() },
        { label: '关闭右侧标签页', icon: 'i-mdi:arrow-expand-right', handle: () => tabbar.closeRightSide(), disabled: !tabbar.checkCloseRightSide() },
      ],
      ...
      <div class="max-h-80 w-40 flex flex-col gap-2">
        ...
        <FaScrollArea :scrollbar="false" mask gradient-color="hsl(var(--popover))">
          <div ref="dropdownTabContainerRef" class="tabs space-y-1" :class="{ dragging: isDragging }">
            ...
            >
              <div :key="element.tabId" class="title" :title="element.customTitleList.find(item => item.fullPath === element.fullPath)?.title || (element.title?.toString() ?? '')" @click="handleClick(element.fullPath)">
                ...
                {{ element.customTitleList.find(item => item.fullPath === element.fullPath)?.title || (element.title?.toString() ?? '') }}
              </div>
