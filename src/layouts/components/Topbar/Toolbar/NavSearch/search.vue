...
function getDisplayTitle(titleOrKey: string | (() => any) = '未命名') {
  if (typeof titleOrKey === 'function') {
    return titleOrKey()
  }
  return titleOrKey
}
...
const resultList = computed(() => {
  ...
  result = sourceList.value.filter((item) => {
    ...
    if (searchInput.value !== '') {
      ...
      if (
        getDisplayTitle(item.title).toString().includes(searchInput.value)
        || (match(getDisplayTitle(item.title).toString(), searchInput.value, { continuous: true })?.length ?? 0) > 0
      ) {
        flag = true
      }
      if (routeStore.getRouteMatchedByPath(item.path).some((b) => {
        return getDisplayTitle(b.meta?.title).toString().includes(searchInput.value)
          || (match(getDisplayTitle(b.meta?.title).toString(), searchInput.value, { continuous: true })?.length ?? 0) > 0
      })) {
        flag = true
      }
    }
...
<template>
  <FaModal
    ...
    header-class="p-0"
    footer-class="p-0"
    @opened="searchInputFocused = true"
  >
    <template #header>
      <div class="h-12 flex flex-shrink-0 items-center">
        ...
        <input
          ref="searchInputRef"
          v-model="searchInput"
          placeholder="支持标题、拼音(首字母)、URL模糊查询"
          class="h-full w-full border-0 rounded-md bg-transparent text-base text-foreground focus-outline-none placeholder-foreground/30"
          @keydown.esc.prevent="isShow = false"
    ...
    <FaScrollArea ref="searchResultRef">
      <template v-if="resultList.length > 0">
        <div
          ...
          class="p-4"
          :data-index="index"
          @click="pageJump(item.path, item.query, item.link)"
          @mouseover="actived = index"
        >
          <a
            class="flex cursor-pointer items-center border rounded-lg"
          ...
          >
            ...
            <div class="flex flex-1 flex-col gap-1 truncate border-s px-4 py-3">
              ...
              <Breadcrumb v-if="routeStore.getRouteMatchedByPath(item.path).length" class="truncate">
                <BreadcrumbItem
                  v-for="(bc, bcIndex) in routeStore.getRouteMatchedByPath(item.path)"
                  :key="bcIndex"
                  class="text-xs"
                >
                  {{ getDisplayTitle(bc.meta?.title) }}
                </BreadcrumbItem>
              </Breadcrumb>
            </div>
          </a>
