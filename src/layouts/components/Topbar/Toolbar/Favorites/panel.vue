  ...
  <div class="favorites-container w-80">
    <div class="flex items-center justify-between px-4 py-3">
      ...
      <template v-if="favoritesStore.canAdd(route.fullPath)">
        <FaIcon v-if="!favoritesStore.isAdd(route.fullPath)" class="cursor-pointer text-xl" name="i-mdi:star-plus-outline" title="添加到收藏夹" @click="favoritesStore.add(route)" />
        <FaIcon v-else class="cursor-pointer text-xl" name="i-mdi:star-remove" title="从收藏夹删除" @click="favoritesStore.remove(route.fullPath)" />
      </template>
    </div>
    <FaScrollArea class="max-h-220px">
      <div v-if="favoritesStore.list.length > 0" ref="favoritesContainerRef" class="flex flex-wrap items-center justify-between gap-2 px-4 pb-4">
        <div v-for="item in favoritesStore.list" :key="item.fullPath" :title="(item.title?.toString() ?? '')" class="group draggable-item relative w-[calc(50%-0.25rem)] flex cursor-pointer items-center gap-1 rounded-lg px-2 py-2 ring-1 ring-border ring-inset hover-bg-accent" @click="handleChoose(item.fullPath)">
