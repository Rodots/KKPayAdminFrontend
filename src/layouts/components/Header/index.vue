    ...
    <header v-if="settingsStore.mode === 'pc' && ['head', 'only-head', 'head-panel'].includes(settingsStore.settings.menu.mode)">
      <div class="header-container">
        ...
        <FaScrollArea :scrollbar="false" mask horizontal gradient-color="var(--g-header-bg)" class="menu-container h-full flex-1">
          ...
          <!-- 顶部精简模式 -->
          <Menu
            v-else-if="settingsStore.settings.menu.mode === 'only-head'" :menu="menuStore.allMenus" :value="route.meta.activeMenu || route.path" mode="horizontal" show-collapse-name collapse :dark="settingsStore.settings.menu.enableDark" :direction="settingsStore.settings.app.direction" class="menu" :class="{
              [`menu-active-${settingsStore.settings.menu.style}`]: settingsStore.settings.menu.style !== '',
            }"
          />
          <!-- 顶部面板模式 -->
          <MenuPanel
            v-else-if="settingsStore.settings.menu.mode === 'head-panel'" :menu="menuStore.allMenus" :value="route.meta.activeMenu || route.path" mode="horizontal" show-collapse-name collapse :dark="settingsStore.settings.menu.enableDark" :direction="settingsStore.settings.app.direction" class="menu" :class="{
              [`menu-active-${settingsStore.settings.menu.style}`]: settingsStore.settings.menu.style !== '',
            }"
          />
        </FaScrollArea>
