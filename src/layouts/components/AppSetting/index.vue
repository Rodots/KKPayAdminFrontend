...
const appRadius = computed<number[]>({
  get() {
    return [settingsStore.settings.app.radius ?? 0.5]
  },
  set(value) {
    settingsStore.settings.app.radius = value[0] ?? 0.5
  },
})
    ...
    new Sortable(val, {
      ...
      onUpdate: (e) => {
        if (e.newIndex !== undefined && e.oldIndex !== undefined) {
          const removedItem = settingsStore.settings.toolbar.layout.splice(e.oldIndex, 1)[0]
          if (removedItem !== undefined) {
            settingsStore.settings.toolbar.layout.splice(e.newIndex, 0, removedItem)
          }
        }
      },
    })
...
<template>
  <FaDrawer v-model="isShow" title="应用配置" description="在生产环境中应关闭该模块" :side="settingsStore.settings.app.direction === 'ltr' ? 'right' : 'left'" :footer="isSupported" :destroy-on-close="false" content-class="sm:min-w-md">
    ...
    <div v-if="settingsStore.mode === 'pc'">
      ...
      <div class="setting-item">
        <div class="label">
          暗黑导航栏
          <FaTooltip text="仅在明亮模式下生效">
            <FaIcon name="i-ri:question-line" />
          </FaTooltip>
        </div>
        <FaSwitch v-model="settingsStore.settings.menu.enableDark" :disabled="settingsStore.currentColorScheme !== 'light'" />
      </div>
    </div>
    <div v-if="settingsStore.mode === 'pc'">
      ...
      <div class="setting-item">
        ...
        <div class="flex-center-start gap-1">
          <FaButton
            v-for="(item, index) in [
              { label: '外层', value: 'outer' },
              { label: '内层', value: 'inner' },
            ]" :key="index" :variant="settingsStore.settings.layout.widthModeScope === item.value ? 'default' : 'outline'" size="sm" @click="settingsStore.settings.layout.widthModeScope = (item.value as any)"
          >
            {{ item.label }}
          </FaButton>
        </div>
    ...
    <div>
      ...
      <div class="setting-item">
        ...
        <FaSwitch v-model="settingsStore.settings.toolbar.favorites" :disabled="!settingsStore.settings.toolbar.enable" />
      </div>
      <div v-if="settingsStore.mode === 'pc'" class="setting-item">
        <div class="label">
          面包屑导航
      ...
      <div class="setting-item">
        ...
        <FaSwitch v-model="settingsStore.settings.toolbar.notification" :disabled="!settingsStore.settings.toolbar.enable" />
      </div>
      <div v-if="settingsStore.mode === 'pc'" class="setting-item">
        <div class="label">
          全屏
    ...
    <div>
      ...
      <div class="setting-item">
        ...
        <FaSwitch v-model="settingsStore.settings.app.enableFeedback" />
      </div>
      <div class="setting-item">
        <div class="label">
          文本方向
...
.app-width-mode {
  ...
  .mode {
    --uno: relative w-20 h-12 rounded-2 ring-1 ring-border cursor-pointer transition of-hidden;
    ...
    &-center {
      &::before {
        --uno: content-empty absolute w-7/10 h-full left-1/2 -translate-x-1/2 bg-primary/20;
      }
