...
const isCollapse = computed(() => {
  if (settingsStore.mode === 'pc') {
    if (settingsStore.settings.menu.subMenuCollapse && (!props.isHover || (props.isHover && !settingsStore.settings.menu.subMenuAutoCollapse))) {
      return true
    }
    else {
      return false
  ...
  <Transition name="sub-sidebar">
    ...
    >
      ...
      <div v-if="settingsStore.settings.menu.mode === 'single'" class="flex-center px-4 pb-3">
        <AccountButton :only-avatar="isCollapse" dropdown-align="center" :dropdown-side="isCollapse ? (settingsStore.settings.app.direction === 'rtl' ? 'left' : 'right') : 'top'" button-variant="secondary" :class="{ 'w-full': !isCollapse }" />
      </div>
