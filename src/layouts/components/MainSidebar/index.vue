  ...
  <Transition name="main-sidebar">
    <div v-if="['side', 'only-side', 'side-panel'].includes(settingsStore.settings.menu.mode) || (settingsStore.mode === 'mobile' && settingsStore.settings.menu.mode !== 'single')" class="main-sidebar-container">
      ...
      <FaScrollArea :scrollbar="false" mask gradient-color="var(--g-main-sidebar-bg)" class="menu flex-1 overscroll-contain">
        ...
        <!-- 侧边栏精简模式 -->
        <Menu
          v-else-if="settingsStore.settings.menu.mode === 'only-side'" :menu="menuStore.allMenus" :value="route.meta.activeMenu || route.path" show-collapse-name collapse :dark="settingsStore.settings.menu.enableDark" :direction="settingsStore.settings.app.direction" class="-mt-2" :class="{
            [`menu-active-${settingsStore.settings.menu.style}`]: settingsStore.settings.menu.style !== '',
          }"
        />
        <!-- 侧边栏面板模式 -->
        <MenuPanel
          v-else-if="settingsStore.settings.menu.mode === 'side-panel'" :menu="menuStore.allMenus" :value="route.meta.activeMenu || route.path" show-collapse-name collapse :dark="settingsStore.settings.menu.enableDark" :direction="settingsStore.settings.app.direction" class="-mt-2" :class="{
            [`menu-active-${settingsStore.settings.menu.style}`]: settingsStore.settings.menu.style !== '',
          }"
        />
      </FaScrollArea>
