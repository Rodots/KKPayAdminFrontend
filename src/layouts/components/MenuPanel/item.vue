...
<template>
  ...
  >
    <router-link v-slot="{ href, navigate }" custom :to="(uniqueKey.at(-1) ?? '') + (item.meta?.query ? `?${qs.stringify(item.meta?.query)}` : '')">
      <FaTooltip :disabled="level !== 0 || subMenu" :text="typeof item.meta?.title === 'function' ? item.meta.title() : item.meta?.title" :side="rootMenu.props.mode === 'vertical' ? 'right' : 'bottom'" class="h-full w-full">
        <component
          :is="subMenu ? 'div' : 'a'" v-bind="{
            ...
            ...(level !== 1 || (level === 1 && !subMenu)
              ? {
                'text-[var(--g-sub-sidebar-menu-color)] transition-colors': true,
