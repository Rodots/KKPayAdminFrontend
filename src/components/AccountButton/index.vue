        ...
        <div class="flex-center-start gap-2">
          ...
          <div class="space-y-1">
            ...
            <div class="text-xs text-secondary-foreground/50 font-normal">
              {{ (userStore.email === '' || userStore.email === null) ? '电子邮箱未设定' : userStore.email }}
            </div>
          </div>
        </div>
      </div>
    ...
    >
      ...
      <div v-if="!onlyAvatar" class="min-w-0 flex-center-between flex-1 gap-2">
        <div class="flex-1 truncate text-start">
          {{ userStore.role_name }} - {{ userStore.nickname }}
        </div>
        <FaIcon name="i-material-symbols:expand-all-rounded" />
      </div>
    </FaButton>
  </FaDropdown>
  <FaModal v-model="isProfileShow" align-center :header="false" :footer="false" :close-on-click-overlay="false" :close-on-press-escape="false" class="h-600px max-w-xl overflow-hidden" content-class="min-h-full p-0 flex">
    <Profile />
  </FaModal>
</template>
